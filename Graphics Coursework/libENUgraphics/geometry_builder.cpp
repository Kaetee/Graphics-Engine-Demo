...
namespace graphics_framework
{
	...
	geometry geometry_builder::create_cylinder(const unsigned int stacks, const unsigned int slices, const glm::vec3 &dims)
	{
		// Type of geometry generated will be triangles
		geometry geom;
		geom.set_type(GL_TRIANGLES);
		...
		auto circ = glm::pi<float>() * ((3.0f * (dims.x + dims.z)) - (sqrtf((3.0f * dims.x + dims.z) * (dims.x + 3.0f * dims.z))));
		...
		for (unsigned int i = 0; i < stacks; ++i)
		{
			...
			for (unsigned int j = 0; j < slices; ++j)
			{
				...
				coords[3] = glm::vec2((-delta_width * (j + 1)) / glm::pi<float>() , dims.y - ((delta_height * (i + 1) * dims.y) / 2.0f));
	...
	geometry geometry_builder::create_torus(const unsigned int stacks, const unsigned int slices, const float ring_radius, const float outer_radius)
	{
		// Type of geometry generated will be triangles
		geometry geom;
		geom.set_type(GL_TRIANGLES);
		...
		for (unsigned int i = 0; i < stacks; ++i)
		{
			...
			for (unsigned int j = 0; j <= slices; ++j)
			{
				...
				coords[0] = glm::vec2((static_cast<float>(i) / static_cast<float>(stacks)) * outer_circ / glm::pi<float>(), (static_cast<float>(j) / static_cast<float>(slices)) * ring_circ / glm::pi<float>());
				// Vertex 1
				verts[1] = glm::vec3(sin(a1) * r, sin(j * delta_slice) * ring_radius, cos(a1) * r);
				norms[1] = glm::normalize(glm::vec3(sin(a1) * c, cos(a1) * c, sin(b)));
				coords[1] = glm::vec2((static_cast<float>(i + 1) / static_cast<float>(stacks)) * outer_circ / glm::pi<float>(), (static_cast<float>(j) / static_cast<float>(slices)) * ring_circ / glm::pi<float>());
				// Vertex 2
				...
				coords[2] = glm::vec2((static_cast<float>(i) / static_cast<float>(stacks)) * outer_circ / glm::pi<float>(), (static_cast<float>(j + 1) / static_cast<float>(slices)) * ring_circ / glm::pi<float>());
				// Vertex 3
				verts[3] = glm::vec3(sin(a1) * r, sin((j + 1) * delta_slice) * ring_radius, cos(a1) * r);
				norms[3] = glm::normalize(glm::vec3(sin(a1) * c, cos(a1) * c, sin(b)));
				coords[3] = glm::vec2((static_cast<float>(i + 1) / static_cast<float>(stacks)) * outer_circ / glm::pi<float>(), (static_cast<float>(j + 1) / static_cast<float>(slices)) * ring_circ / glm::pi<float>());
	...
	geometry geometry_builder::create_plane(const unsigned int width, const unsigned int depth, const float scale)
	{
		// Type of geometry generated will be triangles
		geometry geom;
		geom.set_type(GL_TRIANGLES);
		...
		for (float x = 0; x < width; ++x)
		{
			for (float z = 0; z < depth; ++z)
			{
				...
				verts[0] = glm::vec3((-static_cast<float>(width) * scale) / 2.0f + (x * scale), 0.0f, (static_cast<float>(depth) * scale) / 2.0f - (z * scale));
				verts[1] = glm::vec3((-static_cast<float>(width) * scale) / 2.0f + ((x + 1) * scale), 0.0f, (static_cast<float>(depth) * scale) / 2.0f - (z * scale));
				verts[2] = glm::vec3((-static_cast<float>(width) * scale) / 2.0f + (x * scale), 0.0f, (static_cast<float>(depth) * scale) / 2.0f - ((z + 1) * scale));
				verts[3] = glm::vec3((-static_cast<float>(width) * scale) / 2.0f + ((x + 1) * scale), 0.0f, (static_cast<float>(depth) * scale) / 2.0f - ((z + 1) * scale));
				...
				// Triangle 1
				positions.push_back(verts[0]);
				tex_coords.push_back(glm::vec2(x / scale, z / scale) / 2.0f);
				positions.push_back(verts[3]);
				tex_coords.push_back(glm::vec2((x + 1) / scale, (z + 1) / scale) / 2.0f);
				positions.push_back(verts[2]);
				tex_coords.push_back(glm::vec2(x / scale, (z + 1) / scale) / 2.0f);
				// Triangle 2
				positions.push_back(verts[0]);
				tex_coords.push_back(glm::vec2(x / scale, z / scale) / 2.0f);
				positions.push_back(verts[1]);
				tex_coords.push_back(glm::vec2((x + 1) / scale, z / scale) / 2.0f);
				positions.push_back(verts[3]);
				tex_coords.push_back(glm::vec2((x + 1) / scale, (z + 1) / scale) / 2.0f);
